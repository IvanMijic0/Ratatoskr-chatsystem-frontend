		...
		const initEthereum = async () => {
			try {
				if ( window.ethereum ) {
					...
					console.log(ABI);
					const contract = new ethers.Contract(nftAddress, ABI, await signer);
			...
			try {
				if ( contract ) {
					...
					const formattedNFTData = rawNFTData
						.filter(( nft: any ) => hasNonEmptyValues(nft))
						.map(( nft: {
							name: any;
							description: any;
							price: any;
							...
							return {
								id: typeof nft[nft.length - 1] === 'string' && nft[nft.length - 1].endsWith('n') ? Number(nft[nft.length - 1]) : Number(nft[nft.length - 1]),
								name: nft.name,
		...
		<Box className={ classes.background }
			...
			<Grid container spacing={ 3 } sx={ { textAlign: 'center' } }>
				<Grid container item xs={ 12 } sx={ { display: 'flex', justifyContent: 'center' } }>
					<Button component="label" variant="contained" onClick={ handleModalOpen }
							startIcon={ <CloudUploadIcon/> }>Add New NFT</Button>
